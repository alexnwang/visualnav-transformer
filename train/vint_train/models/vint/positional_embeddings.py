...
def get_3d_sincos_pos_embed(embed_dim, grid_size, grid_depth, n_prefix_tokens=0, cls_token=False, uniform_power=False):
    """
    ...
    if n_prefix_tokens > 0:
        prefix_tokens_embed = get_1d_sincos_pos_embed_from_grid(embed_dim, np.arange(n_prefix_tokens, dtype=float))
        pos_embed = pos_embed.reshape(grid_depth, grid_size**2, embed_dim)
        pos_embed = np.concatenate([prefix_tokens_embed[None].repeat(grid_depth, axis=0), pos_embed], axis=1)
